cluster_plus_ultra_old<-function(mapData=NULL,
                             ...
                             return.plot=TRUE, return.stats=FALSE, return_data=TRUE, gene_annots = NULL) {
  ...
  if(annotate_genes) {
    ...
    for(annotation_count in 1:(ncol(geneannot)-2)){ #don't grab the last column, which is used for ordering
      ...
      if(!is.numeric(geneannot[,annotation_count+1]) && length(unique(geneannot[,annotation_count+1]))>gene_annots){
        
        topannots<-data.frame(sort(table(geneannot[,annotation_count+1]),decreasing=TRUE)[1:gene_annots])
  ...
  for(annotation_count in 1:(ncol(tissueannot)-2)){ #don't grab the last column, which is used for ordering
    ...
    if(!is.numeric(tissueannot[,annotation_count+1]) && length(unique(tissueannot[,annotation_count+1]))>gene_annots){
      
      topannots<-data.frame(sort(table(tissueannot[,annotation_count+1]),decreasing=TRUE)[1:gene_annots])
